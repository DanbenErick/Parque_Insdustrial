...
const PeriodosSettingsTab = () => {
  ...
  return (
    <div className="animate-in fade-in space-y-lg">
      <div className="flex justify-between items-end mb-lg">
        <div>
          <h3 className="font-headline-sm font-bold text-on-surface mb-xs">Periodos de Facturación</h3>
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-surface-container-low border-b border-outline-variant">
                  <th className="py-3 px-4 text-xs font-bold text-on-surface-variant uppercase tracking-wider">Mes/Año</th>
                  <th className="py-3 px-4 text-xs font-bold text-on-surface-variant uppercase tracking-wider">Tarifa kWh</th>
                  <th className="py-3 px-4 text-xs font-bold text-on-surface-variant uppercase tracking-wider">Mant. Normal</th>
                  <th className="py-3 px-4 text-xs font-bold text-on-surface-variant uppercase tracking-wider">Mant. TR</th>
                  <th className="py-3 px-4 text-xs font-bold text-on-surface-variant uppercase tracking-wider">Inicio / Fin</th>
                  <th className="py-3 px-4 text-xs font-bold text-on-surface-variant uppercase tracking-wider text-right">Acciones</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-outline-variant">
                {periodosFiltrados.map((p) => (
                  <tr key={p.id} className="hover:bg-surface-container-lowest transition-colors">
                    ...
                    <td className="py-4 px-4 font-data-mono font-bold">
                      S/ {Number(p.tarifa_kwh).toFixed(4)}
                    </td>
                    <td className="py-4 px-4 font-data-mono">
                      S/ {Number(p.tarifa_mantenimiento_normal).toFixed(2)}
                    </td>
                    <td className="py-4 px-4 font-data-mono text-primary">
                    ...
                    <td className="py-4 px-4 text-sm text-on-surface-variant">
                      ...
                      <span className="text-xs opacity-70">a</span> {new Date(p.fecha_fin).toLocaleDateString()}
                    </td>
