  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4 animate-in fade-in duration-200">
      <div className="bg-surface w-full max-w-xl rounded-2xl shadow-2xl flex flex-col overflow-hidden border border-outline-variant animate-in zoom-in-95 duration-200">
        ...
        <div className="px-4 sm:px-6 py-4 sm:py-5 border-b border-outline-variant flex justify-between items-start sm:items-center gap-4 bg-surface relative overflow-hidden shrink-0">
          ...
          <div className="flex items-center gap-3 sm:gap-4 relative z-10">
            <div className="hidden sm:flex w-12 h-12 rounded-2xl bg-primary items-center justify-center text-on-primary shadow-lg shadow-primary/20 shrink-0">
              <span className="material-symbols-outlined text-[24px]" translate="no">upload_file</span>
            </div>
            <div>
        ...
        {/* Body */}
        <div className="p-4 sm:p-6 flex flex-col gap-4 sm:gap-6 max-h-[80vh] sm:max-h-[85vh] overflow-y-auto custom-scrollbar bg-surface-container-lowest/50">
          {!results ? (
            <>
              {/* Sección 1: Descarga */}
        ...
        <div className="px-4 sm:px-6 py-4 border-t border-outline-variant bg-surface-container flex flex-col-reverse sm:flex-row justify-end gap-3 shrink-0">
          {results ? (
            <button type="button" onClick={onClose} className="w-full sm:w-auto px-6 py-2.5 bg-primary text-on-primary text-sm font-bold rounded-xl shadow-md shadow-primary/20 hover:shadow-lg hover:-translate-y-0.5 transition-all">
              Cerrar y ver Socios
            </button>
          ) : (
            <>
              <button type="button" onClick={onClose} className="w-full sm:w-auto px-5 py-2.5 text-sm font-bold text-on-surface-variant hover:bg-surface-variant rounded-xl transition-colors">
                Cancelar
              </button>
              <button
                type="button"
