...
export const ReadingActions = ({ errors, isSaving }) => (
  <div className="mt-2">
    ...
    <button type="submit" disabled={isSaving || errors.length > 0} className={`w-full py-2.5 rounded-lg font-bold text-sm flex items-center justify-center gap-2 transition-all shadow-sm ${errors.length === 0 ? 'bg-primary text-on-primary hover:opacity-90 hover:shadow-md' : 'bg-surface-container-highest text-on-surface-variant cursor-not-allowed opacity-80'}`}><span className={`material-symbols-outlined text-[18px] ${isSaving ? 'animate-spin' : ''}`} translate="no">{isSaving ? 'sync' : 'save'}</span>{isSaving ? 'Guardando...' : 'Guardar Lectura'}</button>
  </div>
);
