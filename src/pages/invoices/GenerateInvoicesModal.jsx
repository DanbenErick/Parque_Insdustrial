...
const GenerateInvoicesModal = ({ isOpen, onClose, onSuccess, selectedPeriodoId, periodos }) => {
  ...
  // Cerrar el dropdown de autocompletado cuando se hace clic fuera
  useEffect(() => {
  ...
  useEffect(() => {
    if (isOpen && selectedPeriodoId && periodoSeleccionado) {
      const fetchData = async () => {
        setIsLoading(true);
        try {
          const [medidoresRes, lecturas, usuarios] = await Promise.all([
            api.get('/medidores', { params: { operativo: true } }),
            fetchAllPages('/lecturas', { periodo: periodoSeleccionado.mes_anio }),
            fetchAllPages('/usuarios', { rol_id: 3 })
          ]);
          ...
          setUsuarios(usuarios.filter(u => u.nombre_rol === 'Socio'));
        } catch {
  ...
  if (!periodoSeleccionado) {
    return createPortal(
      ...
      >
        <div
          className="bg-surface p-6 rounded-2xl w-full max-w-md shadow-2xl border border-outline-variant"
          onClick={(e) => e.stopPropagation()}
        >
          <h3 className="font-headline-sm font-bold text-on-surface mb-2">Periodo Inválido</h3>
          <p className="text-sm text-on-surface-variant">No se ha seleccionado un periodo válido para facturación.</p>
          <button onClick={onClose} className="mt-4 w-full bg-primary text-on-primary py-2 rounded-xl font-bold hover:brightness-110 transition-all">
            Cerrar
          </button>
        </div>
      </div>,
  ...
  return createPortal(
    <div
      className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-900/60 backdrop-blur-md p-3 sm:p-4 overflow-hidden animate-in fade-in duration-200"
    ...
    >
      <div
        role="dialog"
        aria-modal="true"
        aria-labelledby="modal-generate-invoices-title"
        className="bg-surface w-full max-w-xl max-h-[calc(100dvh-1.5rem)] sm:max-h-[90vh] rounded-[24px] shadow-2xl flex flex-col overflow-hidden animate-in zoom-in-95 duration-200 my-auto border border-outline-variant/40"
        onClick={(e) => e.stopPropagation()}
        style={{ overscrollBehavior: 'contain' }}
      >

        {/* Header - Fijo */}
        <div className="px-5 sm:px-6 py-4 sm:py-5 border-b border-outline-variant/50 bg-surface-container-lowest/50 flex justify-between items-center relative overflow-hidden shrink-0">
          ...
          <div className="flex items-center gap-3 relative z-10 min-w-0">
            ...
            <div className="min-w-0">
              <h3 id="modal-generate-invoices-title" className="font-headline-sm text-on-surface font-bold truncate">Generar Facturas</h3>
              <p className="text-xs text-on-surface-variant mt-0.5 truncate">Emite los recibos del periodo seleccionado</p>
            </div>
          </div>
          <button
            onClick={onClose}
            ...
            className="w-8 h-8 rounded-full hover:bg-surface-container-high flex items-center justify-center text-on-surface-variant hover:text-on-surface transition-colors disabled:opacity-50 relative z-10 shrink-0"
          >
            <span className="material-symbols-outlined text-[20px]" translate="no">close</span>
          </button>
        </div>

        {/* Content - Scrollable con overscroll contain */}
        <div
          className="p-4 sm:p-6 space-y-4 sm:space-y-6 flex-1 min-h-0 overflow-y-auto custom-scrollbar modal-scroll-area"
          style={{ overscrollBehavior: 'contain', WebkitOverflowScrolling: 'touch' }}
        >

          {/* Resumen del periodo en un solo bloque */}
          <div className="bg-surface-container-lowest border border-outline-variant/60 rounded-xl p-3.5 sm:p-4 flex items-center justify-between shadow-sm relative overflow-hidden shrink-0">
            <div className="absolute left-0 top-0 bottom-0 w-1 bg-primary"></div>
            <div>
              <p className="text-[10px] font-bold text-on-surface-variant uppercase tracking-wider mb-1">Periodo a facturar</p>
              <p className="text-lg sm:text-xl font-bold text-primary">{formatPeriod(periodoSeleccionado.mes_anio)}</p>
            </div>
            <div className="h-10 w-px bg-outline-variant/50 mx-3 sm:mx-4"></div>
            <div>
              ...
              <div className="flex items-center gap-1.5">
                ...
                <p className="font-data-mono font-bold text-sm sm:text-base text-on-surface">
                  S/ {Number(periodoSeleccionado.tarifa_kwh).toFixed(4)} <span className="text-xs font-normal text-on-surface-variant">/ kWh</span>
                </p>
              </div>
            </div>
          </div>
          ...
          {isLoading ? (
            ...
            <>
              {/* Toggle Modo */}
              <div className="flex bg-surface-container-lowest p-1.5 rounded-[14px] border border-outline-variant/50 shadow-sm relative shrink-0">
                <div
              ...
              {modo === 'Masivo' ? (
                <>
                  {/* Status Grid */}
                  <div className="grid grid-cols-3 gap-2 sm:gap-3 animate-in fade-in slide-in-from-bottom-2 duration-300">
                    <div className="bg-surface-container-lowest rounded-xl p-2.5 sm:p-3 text-center border border-outline-variant/50 shadow-sm flex flex-col justify-center">
                      <p className="text-[10px] font-bold text-on-surface-variant uppercase tracking-wider mb-1">Medidores</p>
                      <p className="font-data-mono font-bold text-xl sm:text-2xl text-on-surface leading-none">{totalMedidores}</p>
                    </div>
                    <div className="bg-success/5 border border-success/20 rounded-xl p-2.5 sm:p-3 text-center shadow-sm flex flex-col justify-center">
                      ...
                      <p className="font-data-mono font-bold text-xl sm:text-2xl text-success leading-none">{cantidadLecturas}</p>
                    </div>
                    <div className={`border rounded-xl p-2.5 sm:p-3 text-center shadow-sm flex flex-col justify-center transition-colors ${lecturasFaltantes > 0 ? 'bg-error/5 border-error/20' : 'bg-surface-container-lowest border-outline-variant/50'}`}>
                      ...
                      <p className={`font-data-mono font-bold text-xl sm:text-2xl leading-none ${lecturasFaltantes > 0 ? 'text-error' : 'text-on-surface'}`}>
                        {lecturasFaltantes}
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="animate-in fade-in duration-300">
                    {totalMedidores === 0 ? (
                      <div className="bg-error/10 border border-error/20 px-3.5 sm:px-4 py-3 rounded-xl flex items-start gap-3">
                      ...
                      <div className="bg-success/10 border border-success/20 px-3.5 sm:px-4 py-3 rounded-xl flex items-start gap-3">
                ...
                <div className="bg-surface-container-lowest border border-outline-variant/60 p-4 sm:p-5 rounded-xl shadow-sm animate-in fade-in slide-in-from-bottom-2 duration-300">
                  ...
                  <div className="relative" ref={dropdownRef}>
                    <div className="relative">
                      ...
                      <input
                        ...
                        className="w-full pl-10 pr-9 py-2.5 bg-surface border border-outline-variant rounded-lg text-sm font-medium focus:border-primary focus:ring-2 focus:ring-primary/20 outline-none transition-all"
                      />
                      {searchTerm && (
                        <button
                          type="button"
                          className="absolute right-2.5 top-1/2 -translate-y-1/2 text-on-surface-variant hover:text-on-surface bg-surface-variant/50 hover:bg-surface-variant w-5 h-5 rounded-full flex items-center justify-center transition-colors"
                          onClick={() => {
                            setSearchTerm('');
                            setSelectedUsuarioId('');
                            setSelectedMedidorId(null);
                  ...
                  <div className="mt-3.5 bg-secondary/5 px-3 py-2 rounded-lg border border-secondary/10 flex items-start gap-2">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5 sm:gap-3 pt-1">
                <div className="bg-surface-container-lowest border border-outline-variant/40 p-2.5 sm:p-3 rounded-xl flex items-start gap-2.5 sm:gap-3">
                  <span className="material-symbols-outlined text-secondary text-[18px] sm:text-[20px] mt-0.5 shrink-0" translate="no">info</span>
                ...
                <div className="bg-error/5 border border-error/10 p-2.5 sm:p-3 rounded-xl flex items-start gap-2.5 sm:gap-3">
                  <span className="material-symbols-outlined text-error text-[18px] sm:text-[20px] mt-0.5 shrink-0" translate="no">history</span>
        ...
        {/* Footer Actions - Fijo */}
        <div className="px-5 sm:px-6 py-3.5 sm:py-4 bg-surface-container-lowest border-t border-outline-variant/50 flex flex-col sm:flex-row gap-2 sm:gap-2.5 sm:justify-end sm:items-center shrink-0">
          <button
            type="button"
            onClick={onClose}
            disabled={isProcessing}
            className="py-2 px-4 rounded-xl font-bold text-sm border border-outline-variant text-on-surface-variant hover:bg-surface-container transition-colors disabled:opacity-50 order-last sm:order-first"
          >
            Cancelar
          </button>
          ...
          {modo === 'Masivo' ? (
            <>
              <button
                ...
                className="py-2 px-3.5 rounded-xl font-bold text-xs sm:text-sm bg-secondary/15 text-secondary border border-secondary/30 hover:bg-secondary/25 hover:shadow-sm transition-all active:scale-[0.98] flex items-center justify-center gap-1.5 disabled:opacity-50 disabled:cursor-not-allowed disabled:active:scale-100 disabled:hover:shadow-none"
              ...
              <button
                ...
                className="py-2 px-4 rounded-xl font-bold text-xs sm:text-sm bg-primary text-on-primary hover:brightness-110 hover:shadow-md transition-all active:scale-[0.98] flex items-center justify-center gap-1.5 disabled:opacity-50 disabled:cursor-not-allowed disabled:active:scale-100 disabled:hover:shadow-none"
            ...
            <button
              ...
              className="py-2 px-6 rounded-xl font-bold text-sm bg-primary text-on-primary hover:brightness-110 hover:shadow-md transition-all active:scale-[0.98] flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed disabled:active:scale-100 disabled:hover:shadow-none"
            >
              {isProcessing ? (
                <>
                  <span className="material-symbols-outlined animate-spin text-[18px]" translate="no">sync</span>
