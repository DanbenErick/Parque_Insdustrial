...
const CargoFormModal = ({ formData, setFormData, periodos, onClose, onSubmit, isSubmitting }) => {
  ...
  const handleFieldChange = useCallback((field) => (e) => {
    ...
    setFormData(prev => field === 'tipo' && value === 'Multa'
      ? { ...prev, tipo: value, es_global: false, periodos_ids: [] }
      : { ...prev, [field]: value });
  }, [setFormData]);
      ...
      >
        ...
        <form onSubmit={onSubmit} className="flex flex-col flex-grow overflow-hidden">
          <div className="p-4 overflow-y-auto custom-scrollbar flex-grow space-y-4">
              ...
              <div className="md:col-span-2 pt-2 border-t border-outline-variant/30 mt-2 grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                {formData.tipo === 'Costo' ? (
                  ...
                  <div className="rounded-lg border border-error/20 bg-error/5 px-3 py-2">
                    <p className="text-xs font-bold text-error">Aplicación manual</p>
                    <p className="text-[10px] text-on-surface-variant">Esta multa solo se cobrará cuando un operador la aplique a un recibo.</p>
                  </div>
                )}
              </div>
            </div>
            ...
            {/* Periodos section */}
            {formData.tipo === 'Costo' && !formData.es_global && (
